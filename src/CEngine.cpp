...
const char *outchamsText[MAX_CHAMSOUT] =
{
 	"Off",
 	"OutChams Original",
 	"OutChams Marble",
...
const char *predictionTypeText[PRED_MAX] =
{
	"Off",
 	"Ping",
  	"Manual",
...
void CEngine::RegisterMedia()
{
	...
	// Chams
	eth32.cg.media.chamsOut[CHAMSOUT_ORIG] = Syscall.R_RegisterShader("outChams");
	eth32.cg.media.chamsOut[CHAMSOUT_MARBLE] = Syscall.R_RegisterShader("outChamsmarble");
	eth32.cg.media.chamsOut[CHAMSOUT_CRYSTAL] = Syscall.R_RegisterShader("outChamscrystal");
	eth32.cg.media.chamsOut[CHAMSOUT_THERMAL] = Syscall.R_RegisterShader("outChamsthermal");
	eth32.cg.media.chamsOut[CHAMSOUT_QUAD] = Syscall.R_RegisterShader("outChamsquad");
	eth32.cg.media.chamsOut[CHAMSOUT_PLASMA] = Syscall.R_RegisterShader("outChamsplasma");
	eth32.cg.media.chamsOut[CHAMSOUT_PLASTIC] = Syscall.R_RegisterShader("outChamsplastic");
	eth32.cg.media.chams[CHAMS_KERAMIC] = Syscall.R_RegisterShader("keramicChams");
	eth32.cg.media.chams[CHAMS_CRISPWAVE] = Syscall.R_RegisterShader("CrispWaveChams");
	...
	eth32.cg.media.chams[CHAMS_THERMAL] = Syscall.R_RegisterShader("thermalChams");
        eth32.cg.media.chams[CHAMS_CRYSTALB] = Syscall.R_RegisterShader("crystalBChams");
	eth32.cg.media.chams[CHAMS_XRAY] = Syscall.R_RegisterShader("xrayChams");
	eth32.cg.media.chams[CHAMS_GRID] = Syscall.R_RegisterShader("gridChams");
	eth32.cg.media.chams[CHAMS_WAVE] = Syscall.R_RegisterShader("waveChams");
	eth32.cg.media.chams[CHAMS_CRISPY] = Syscall.R_RegisterShader("crispyChams");
	eth32.cg.media.chams[CHAMS_MARBLE] = Syscall.R_RegisterShader("marbleChams");
	...
	eth32.cg.media.weaponIcons[WP_MAPMORTAR] = Syscall.R_RegisterShaderNoMip("icons/iconw_grenade_1_select");
	...
	eth32.cg.media.weaponIcons[WP_SMOKE_BOMB] = Syscall.R_RegisterShaderNoMip("icons/iconw_dynamite_1_select");
	eth32.cg.media.weaponIcons[WP_TRIPMINE] = Syscall.R_RegisterShaderNoMip("icons/iconw_dynamite_1_select");
	...
	eth32.cg.media.weaponIcons[WP_GRENADE_LAUNCHER] = Syscall.R_RegisterShaderNoMip("icons/iconw_grenade_1_select");
	...
	eth32.cg.media.weaponIcons[WP_GRENADE_PINEAPPLE] = Syscall.R_RegisterShaderNoMip("icons/iconw_pineapple_1_select");
	...
	eth32.cg.media.weaponIcons[WP_DYNAMITE] = Syscall.R_RegisterShaderNoMip("icons/iconw_dynamite_1_select");
	...
	eth32.cg.media.weaponIcons[WP_LANDMINE] = Syscall.R_RegisterShaderNoMip("icons/iconw_landmine_1_select");
	eth32.cg.media.weaponIcons[WP_SATCHEL] = Syscall.R_RegisterShaderNoMip("icons/iconw_satchel_1_select");
...
void playSpreeSound() {	
	if (eth32.settings.randomSpreeSounds) {	// Random Spree sounds
		if ((eth32.cg.cgTime - eth32.lastKillTime) < (eth32.settings.mkResetTime)) {
			...
			int level = eth32.killSpreeCount - 2; // never '< 0' because first time here is with 2 kills
			...
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[sound], CHAN_LOCAL_SOUND, 230);
		}
	} 
	else { // Sequential Spree sounds
		// Multikills
		if (eth32.killSpreeCount == 2)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_DOUBLEKILL1], CHAN_LOCAL_SOUND, 230); 
		else if (eth32.killSpreeCount == 3)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_TRIPLEKILL], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount == 4)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_MULTIKILL3], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount == 5)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_MEGAKILL], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount == 6)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_ULTRAKILL1], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount == 7)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_MONSTERKILL2], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount == 8)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_UNREAL], CHAN_LOCAL_SOUND, 230);
		else if (eth32.killSpreeCount >= 9)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_HOLYSHIT], CHAN_LOCAL_SOUND, 230);
		
		// Current Spree	
		if (eth32.cg.self->stats.spree == 5)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_KILLINGSPREE1], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 10)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_MULTIKILL2], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 15)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_DOMINATING1], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 20)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_UNSTOPPABLE1], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 25)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_GODLIKE1], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 30)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_WICKEDSICK], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 35)
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_HOLYSHIT], CHAN_LOCAL_SOUND, 230);
		else if (eth32.cg.self->stats.spree == 40 )
			orig_syscall(CG_S_STARTLOCALSOUND, eth32.cg.media.sounds.spree[SOUND_HOLYSHIT], CHAN_LOCAL_SOUND, 230);
	}
}
		...
		if (attacker >= 0 && attacker < MAX_CLIENTS) {	// make sure attacker wasn't world or tank etc
			...
			if (attacker == eth32.cg.clientNum) {
				...
				if(eth32.settings.killSounds) {
					if ((eth32.cg.cgTime - eth32.lastKillTime) > (eth32.settings.mkResetTime))
						eth32.killSpreeCount = 1;
					else
						eth32.killSpreeCount++;
					
					eth32.lastKillTime = eth32.cg.cgTime;
					
					if (!eth32.settings.randomSpreeSounds) {
						if (eth32.killSpreeCount > 1 || eth32.cg.self->stats.spree >= 5) // Alexplay: Play both multikill and current spree sounds.
							playSpreeSound();
					} else {
						if (eth32.killSpreeCount > 1) // Don't do the same check as above for random sounds, otherwise it crashes.
							playSpreeSound();
					}
				}
...
void CEngine::PreFrame(int serverTime)
{
	...
	playerDrawn = false;
       	...
       	if (eth32.cgMod->type != MOD_ETPRO && eth32.settings.predictionType == PRED_PING) { 
	...
	// Anti TeamKill
	if (eth32.settings.antiTk && eth32.cg.clientNum == eth32.cg.snap->ps.clientNum) {
		int attacker = eth32.cg.snap->ps.persistant[PERS_ATTACKER];
		int slot;
		for (int i=0; i<MAX_CLIENTS; i++) {
			slot = eth32.cg.players[i].clientNum;
			if (slot == attacker) 
...
void CEngine::PostFrame(void)
{
	...
	if (eth32.settings.drawHackVisuals && !Gui.IsLimboPanelUp() && eth32.cg.snap->ps.pm_type != PM_INTERMISSION) {	
		...
		if (eth32.settings.showDamage)
			DrawHitIndicator();
		if (eth32.settings.showReload)
			DrawWarningMessages();
	}
	memset(eth32.refEntities, 0, sizeof(eth32.refEntities));
	eth32.refEntitiesCount = 0;
...
{
	memcpy(&eth32.refEntities[eth32.refEntitiesCount++], re, sizeof(refEntity_t));
	...
	if (pDrawPlayer && (re->reType != RT_SPRITE)) { // this is part of a player and isn't an icon above it's head
		...
		if (drawWeapon) {
			...
			ON_VISUALS {	
				...
				eth32.settings.TDOutLine ? re->renderfx |= RF_DEPTHHACK : re->renderfx &= ~RF_DEPTHHACK;
			}

			if (eth32.settings.weapoutChams) {
                    		ON_VISUALS re->customShader = eth32.cg.media.chamsOut[eth32.settings.weapoutChams];
		...
		if (eth32.settings.outChams) {
			ON_VISUALS re->customShader = eth32.cg.media.chamsOut[eth32.settings.outChams];
...
{
	for (int entityNum=0; entityNum<eth32.refEntitiesCount; entityNum++) {
		refEntity_t* refEnt = &eth32.refEntities[entityNum];
		int x, y;
		int size = eth32.settings.itemEspSize;
		vec4_t color;
...
void CEngine::PlayerEsp(player_t *player)
{
	...
	if (Draw.worldToScreen(espOrigin, &player->screenX, &player->screenY)) {
		...
		if (eth32.settings.classIcon && !Gui.IsLimboPanelUp()) {
			float iconSize, x, y; 		
			vec4_t iconColor;
			...
			if (eth32.settings.classEspType == CLS_STATIC) {
				...
				iconSize = 10000.0f / player->distance;
 				iconSize < 5.0f ? 5.0f : iconSize;
 				iconSize > 8.0f ? 8.0f : iconSize;
				// Position
				y = player->screenY;
				x = player->screenX - (iconSize * 3) / 2;
				y -= iconSize + 2;
